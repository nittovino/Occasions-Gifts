...
const SympathyMemorial = () => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      const pageProducts = allProducts.filter(p => 
        p.occasion_tags?.includes('Memorial')
      );
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#1a2a4a]">
      <Helmet>
        <title>{t('memorialTribute')} - Occasions Gifts</title>
      </Helmet>
      ...
      <main className="flex-1 container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center text-sm text-slate-400 mb-8">
          ...
          <ChevronRight className="h-4 w-4 mx-2" />
          <span className="text-[#d4af37] font-medium">{t('memorialTribute')}</span>
        </div>

        <div className="mb-6">
          <h1 className="text-3xl font-serif font-bold text-white mb-2">{t('memorialTribute')}</h1>
          <p className="text-slate-400">Lasting tributes and respectful remembrances.</p>
        </div>
...
export default SympathyMemorial;