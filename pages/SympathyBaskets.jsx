...
const SympathyBaskets = () => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      // Filter for baskets/food
      const pageProducts = allProducts.filter(p => 
        p.occasion_tags?.includes('Support Baskets') || 
        p.category === 'Gift Baskets' ||
        p.category === 'Food & Beverage'
      );
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#1a2a4a]">
      <Helmet>
        <title>{t('supportBaskets')} - Occasions Gifts</title>
      </Helmet>
      ...
      <main className="flex-1 container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center text-sm text-slate-400 mb-8">
          ...
          <ChevronRight className="h-4 w-4 mx-2" />
          <span className="text-[#d4af37] font-medium">{t('supportBaskets')}</span>
        </div>

        <div className="mb-6">
          <h1 className="text-3xl font-serif font-bold text-white mb-2">{t('supportBaskets')}</h1>
           <p className="text-slate-400">Comforting food and care packages.</p>
        </div>

        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[1,2,3,4].map(i => <div key={i} className="h-80 bg-slate-800 animate-pulse rounded-xl" />)}
          </div>
        ) : filteredProducts.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {filteredProducts.map(p => <SympathyProductCard key={p.id} product={p} />)}
          </div>
        ) : (
...
export default SympathyBaskets;